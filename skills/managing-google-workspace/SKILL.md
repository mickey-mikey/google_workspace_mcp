...
If `settings.local.json` already exists, they should merge into the existing `env` object, not replace it. `PYTHONIOENCODING=utf-8` prevents encoding errors on Windows. These env vars are inherited by all MCP servers and CLI processes that Claude Code spawns.
...
- Credentials are cached in `~/.claude/.credentials` for future sessions
